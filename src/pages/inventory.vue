<script setup>
import { CameraOutline, Api, Twitch, Calculator, CheckboxMarkedCircleOutline } from 'mdue'
...
const snapshot = computed(()=>{ 
  ...
  const newD = Object.keys(api).map(g=>{
    ...
    return { ... game, what: game?.what ?? g, api: gAPI, redeems: gRed, enabled }
  })
  return newD
    ?.filter(g=>g?.enabled)
...
<template>
  <section>
    <article class="px-2 max-w-fit m-2 flex flex-row gap-2">
      <label role="button" :class="{'font-bold':onlyChanged}" class="hover:bg-purple-950 rounded-xl p-2">
        {{ onlyChanged ? '🖱️ to show all 🎮' : '🖱️ to show 🎮 w/ redeems' }}
        <input type="checkbox" v-model="onlyChanged" class="hidden" />
      </label>
      <label role="button" :class="{'font-bold':mismatched}" class="hover:bg-purple-950 rounded-xl p-2">
        {{ mismatched ? 'showing only "misMatched" (🖱️ for all 🎮)' : 'showing all 🎮 (🖱️ for mismatched only)' }}
    ...
    <table>
      ...
      <tr v-for="game of snapshot?.filter(g=> 
        ...
        && (!findUser ? true : g?.redeems?.find(r=>r?.user==findUser)?.time )
        ...
        <td class="num calc" :class="(game?.qty - (game?.redeems?.length??0)) == game?.api ? 'good':'else'">
          ...
          <span v-else>{{  (game?.qty - game?.redeems?.length) - game?.api  }}?</span>
        </td>
        <td class=redeemRow>
          <details v-if="game?.redeems?.length" :open="mismatched">
